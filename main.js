...
function revealMyInfo(myInfo, className, color, animationName) {
  ...
  if (animationName === "bgFadeOut") {
    ...
    htmlElement.style.animationName = animationName;
    console.log(htmlElement.style.animationName);
  }
}

function generate(nameClassName, birthClassName, colorClassName) {
  let nameElement = document.querySelector(nameClassName);
  let birthElement = document.querySelector(birthClassName);
  yourName = nameElement.value;
  birth = birthElement.value;
  let nHash = SHA256(yourName);
  let bHash = SHA256(birth);
  let hString = nHash + bHash;
  let hHash = SHA256(hString);
  let sum = 0;
  for (let i = 0; i < 11; i++) {
    sum = sum + parseInt("0x" + hHash.substring(i * 6, i * 6 + 6), 16);
  }
  let colorCode = sum.toString(16).substring(0, 6);
  console.log(colorCode);
  let colorElement = document.querySelector(colorClassName);
  colorElement.src = "https://www.colorhexa.com/" + colorCode + ".png";
}
